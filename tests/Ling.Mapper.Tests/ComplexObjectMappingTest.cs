...
/// <summary>
/// 复杂对象映射测试 - 测试深层嵌套、多层继承等复杂场景
/// </summary>
public static class ComplexObjectMappingTest
{
    public static void Run()
    {
        Console.WriteLine("--- 复杂对象映射测试 ---\n");
    ...
    {
        Console.WriteLine("2. 多层嵌套映射 (5层深度)");
        ...
        try
        {
            ...
            if (target != null)
            {
                ...
                if (target.Employees?.Count == 3)
                {
                    ...
                    Console.WriteLine($"  ? 总工资: ${totalSalary}");
                }
            }
        }
        catch (Exception ex)
    ...
    private static void TestObjectInCollection()
    {
        ...
        try
        {
            ...
            {
                Console.WriteLine($"  ? 产品数量: {target.Count}");
